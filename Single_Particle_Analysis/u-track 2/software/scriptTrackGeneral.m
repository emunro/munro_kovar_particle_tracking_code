...
if(exist('defineScriptTrackGeneralParameters','var') & defineScriptTrackGeneralParameters)
    if exist('gapClosingTimeWindow','var')
        gapCloseParam.timeWindow = gapClosingTimeWindow;    
    else
    ...
    if exist('gapClosingMergeSplit','var')
        gapCloseParam.mergeSplit = gapClosingMergeSplit;    
    else
...
%NEW PARAMETER
parameters.gapExcludeMS = 0; %flag to allow gaps to exclude merges and splits
...
saveResults.filename = 'tracks.mat'; %name of file where input and output are saved
