function r = feature2D(img,lambda,w,masscut,Imin,field,Ihalo)
...
% 		f(:,6): background-subtracted integrated intensity.
...
%%%%%%%%%%%%%    EMM: extra code to measure background-subtracted intensities from the raw image %%%%%%%%%%%
if Ihalo > 0
    twoIhalo = 2*Ihalo;
    avrsqd = rsqd( extent+twoIhalo,extent+twoIhalo);
    avmask = le(avrsqd,(extent/2+Ihalo)^2);
    avmask = avmask-padarray(mask,[Ihalo Ihalo]);
    av = zeros(extent+twoIhalo,extent+twoIhalo,nmax);
end
%%%%%%%%%%%%%

%	Construct the subarray and calculate the mass, squared radius of gyration, eccentricity
for i=1:nmax,    
    %%%%%%%%%%%%%    EMM: code to measure background-subtracted intensities from the raw image %%%%%%%%%%%
    if Ihalo>0
        if fix(yl(i))>Ihalo & fix(yh(i))<ny-Ihalo & fix(xl(i))>Ihalo & fix(xh(i))<nx-Ihalo
            suba(:,:,i) = fracshift( double(img(fix(yl(i)):fix(yh(i)),fix(xl(i)):fix(xh(i)))), -xc(i) , -yc(i) );
            m2(i) = sum(sum(( suba(:,:,i).*mask )));             % mass
            av(:,:,i) = fracshift( double(img(fix(yl(i))-Ihalo:fix(yh(i))+Ihalo,fix(xl(i))-Ihalo:fix(xh(i))+Ihalo)), -xc(i) , -yc(i) );
            av2 = sum(sum(( av(:,:,i).*avmask)))/length(find(avmask));
            m2(i) = m2(i) - length(find(mask))*av2;
        else
            m2(i) = nan;
        end
    else
        suba(:,:,i) = fracshift( double(img(fix(yl(i)):fix(yh(i)),fix(xl(i)):fix(xh(i)))), -xc(i) , -yc(i) );
        m2(i) = sum(sum(( suba(:,:,i).*mask )))/length(find(mask));             % mass
    end
    %%%%%%%%%%%%%
