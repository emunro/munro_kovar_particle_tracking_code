...
%    4/07  CWH Changed the difficult combinatorics cutoff to 2.e+10
%               from 2.e+5.  Also added a wait bar since the program
%               can now take quite a while to complete.
%
...
if nargin==2
    %default values
    memory_b=3; % if mem is not needed set to zero
...
disp('  Tracking Particles (wait for "done" indication) ...');
progtitle = 'Progress in tracking particles';
...
%                end
    ...
    if (ispan == zspan) | (i == z)
        ...
        if quiet~=1
            fs = sprintf('%d of %d done.  Tracking %d particles with %d tracks total', i, z, ntrk, n);
            disp(fs);
%             disp(strcat(num2str(i), ' of ' ,num2str(z), ' done.  Tracking  ',num2str(ntrk),' particles  ', num2str(n),' tracks total'));
    ...
    waitbar(i/z, progbar, strcat(progtitle, ' ...'));
end
close(progbar);
