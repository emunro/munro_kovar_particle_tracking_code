function varargout = Crocker_Pretrack_GUI(varargin)
% Crocker_PRETRACK_GUI MATLAB code for Crocker_Pretrack_GUI.fig
%      Crocker_PRETRACK_GUI, by itself, creates a new Crocker_PRETRACK_GUI or raises the existing
%      singleton*.
%
%      H = Crocker_PRETRACK_GUI returns the handle to a new Crocker_PRETRACK_GUI or the handle to
%      the existing singleton*.
%
%      Crocker_PRETRACK_GUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in Crocker_PRETRACK_GUI.M with the given input arguments.
%
%      Crocker_PRETRACK_GUI('Property','Value',...) creates a new Crocker_PRETRACK_GUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Crocker_Pretrack_GUI_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Crocker_Pretrack_GUI_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Crocker_Pretrack_GUI
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @Crocker_Pretrack_GUI_OpeningFcn, ...
                   'gui_OutputFcn',  @Crocker_Pretrack_GUI_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Crocker_Pretrack_GUI is made visible.
function Crocker_Pretrack_GUI_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Crocker_Pretrack_GUI (see VARARGIN)

% Choose default command line output for Crocker_Pretrack_GUI
...
numFiles = length(dir('fov1/*.tif'));
maxfilestep = 10/(numFiles-1);
minfilestep = 1/(numFiles-1);
...
set(handles.FrameSlider,'Max',numFiles,'SliderStep',[minfilestep maxfilestep]);

handles.im = im;
update_features(hObject,handles);

% UIWAIT makes Crocker_Pretrack_GUI wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Crocker_Pretrack_GUI_OutputFcn(hObject, eventdata, handles) 
...
% Load the new image
fs = sprintf('fov1/fov1_%04d.tif',currval);
handles.im = imread(fs);
guidata(hObject,handles);
% update_features(hObject,handles);
...
if get(hObject,'Value') == 1;
    ...
    theta = 0:0.001:2*pi;
    for c = 1:length(M(:,1))
        cx = M(c,1) + (featsize/2)*cos(theta)*2;
        cy = M(c,2) + (featsize/2)*sin(theta)*2;
